...
from Tools.Directories import SCOPE_PLUGINS, SCOPE_LIBDIR, SCOPE_CURRENT_SKIN, fileCheck, fileReadLine, fileReadLines, resolveFilename, fileExists, fileHas, fileReadLine, pathExists
...
def getRCFile(ext):
	filename = resolveFilename(SCOPE_CURRENT_SKIN, pathjoin("rc_models", "%s.%s" % (BoxInfo.getItem("rcname"), ext)))
	if not isfile(filename):
		filename = resolveFilename(SCOPE_CURRENT_SKIN, pathjoin("rc_models", "dmm.%s" % ext))
	return filename
